...
public class BookServiceImpl implements BookService {
    ...
    public List<Book> findAll() throws ExecutionException, InterruptedException {
        Multi<DbRow> execute = this.dbClient
                .execute(exe -> exe.createQuery("SELECT a.id as author_id, a.first_name, a.last_name, b.id, b.title, b.isbn, b.price " +
                        "FROM public.\"Book\" b JOIN public.\"Author\" a ON b.author_id = a.id").execute());
    ...
    public Book findOne(int id) throws ExecutionException, InterruptedException {
        Optional<DbRow> dbRow = this.dbClient
                .execute(exe -> exe.createGet("SELECT a.id as author_id, a.first_name, a.last_name, b.id, b.title, b.isbn, b.price " +
                        "FROM public.\"Book\" b JOIN public.\"Author\" a ON b.id = a.id WHERE b.id = :id").addParam("id", id).execute()).get();
    ...
    public long save(Book book) {
        ...
        try {
            rowsChanged = this.dbClient
                    .execute(exec -> exec
                            .insert("INSERT INTO public.\"Book\" (author_id, isbn, title, price) VALUES(?, ?, ?, ?)",
    ...
    public long update(Book book) {
        ...
        try {
            rowsChanged = this.dbClient
                    .execute(exec -> exec
                            .update("UPDATE public.\"Book\" SET author_id = ?, isbn = ?, title = ?, price = ? WHERE id = ?",
    ...
    public long delete(int id) {
        ...
        try {
            rowsChanged = this.dbClient
                    .execute(exec -> exec
                            .delete("DELETE FROM public.\"Book\" WHERE id = ?", id
                            )).get();
        } catch (Exception e) {
            e.printStackTrace();
        }
